...
R = QQ[k_(1,1), k_(1,2), k_(1,3), k_(2,2), k_(2,3), k_(3,3), k_(3,4), k_(4,4)]
K = matrix for i from 1 to 4 list for j from 1 to 4 list if ((i < 3 and j > 3) or (i > 3 and j < 3)) then 0 else k_(min(i,j), max(i,j))
f = det K
F = factor f;
degree f

v = matrix {{k_(1,1), k_(1,2), k_(1,3), k_(2,2), k_(2,3), k_(3,3), k_(3,4), k_(4,4)}}
V = (transpose v) ** v;
